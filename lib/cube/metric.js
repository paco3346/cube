...
exports.getter = function(db) {
  ...
  function findOrComputeUnary(expression, start, stop, tier, callback) {
    ...
    function find(start, stop, tier, callback) {
      ...
      }, {projection: metric_fields, options: metric_options.sort, batchSize: metric_options.batchSize}, foundMetrics);
      ...
      function computeFlat(start, stop) {
        filter.t.$gte = start;
        filter.t.$lt = stop;
        type.events.find(filter, {projection: fields, sort: event_options.sort, batchSize: event_options.batchSize}, function(error, cursor) {
          handle(error);
