...
var type_re = /^[a-z][a-zA-Z0-9_]+$/,
    ...
    multi = {multi: true},
    metric_options = {autoIndexId: true};
...
exports.putter = function(db) {
  ...
  function putter(request, callback) {
    ...
    var event = {t: time, d: request.data};
      if ("id" in request) {
          event._id = ObjectID.createFromHexString(request.id);
          event.u = new Date();
      }
...
exports.getter = function(db) {
  ...
  function getter(request, callback) {
    ...
    var filter = {$or: [{t: {$gte: start, $lt: stop}}, {u: {$gte: start}}]};
    ...
    function query(callback) {
      collection(expression.type).events.find(filter, options, function(error, cursor) {
        handle(error);
          ...
          query(function(event) {
            ...
            // the time range, and set a timeout for the next poll.
              ...
              if (!streams.active.length) {
                ...
                filter.$or[0].t.$gte = streams.time;
                filter.$or[1].u.$gte = streams.time;
                filter.$or[0].t.$lt = streams.time = new Date(Date.now() - delay);
